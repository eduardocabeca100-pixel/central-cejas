...
function registrarAgendaDiaApi(app) {
  app.get("/api/agenda-dia", async (req, res) => {
    try {
      ...
      if (dataFiltro) querySupera = querySupera.eq("data_evento", dataFiltro);
      ...
      eventosSupera = (supera.data || []).map(eventoSupera);
      ...
      try {
        ...
        if (dataFiltro) queryManual = queryManual.eq("data", dataFiltro);

        const manual = await queryManual;

        if (manual.error) {
          console.log("⚠️ Agenda manual ainda não disponível:", manual.error.message);
        } else {
          eventosManuais = (manual.data || []).map(eventoManual);
        }
      } catch (manualError) {
        console.log("⚠️ Agenda manual ignorada:", manualError.message);
      }

      const eventos = [...eventosSupera, ...eventosManuais].sort((a, b) => {
  ...
  console.log("✅ Agenda Dia API forçada e ativa.");
}
