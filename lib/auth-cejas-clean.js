...
function salvarSessao(req, usuario, lembrar) {
  ...
  // Sessão principal nova
  req.session.usuario = sessao;

  // Compatibilidade com telas antigas do sistema
  req.session.user = sessao;
  req.session.currentUser = sessao;
  req.session.authUser = sessao;

  // Flags usadas por proteções antigas
  ...
  // Campos soltos usados por páginas antigas
  req.session.email = sessao.email;
  req.session.nome = sessao.nome;
  req.session.name = sessao.nome;
  req.session.cargo = sessao.cargo;
  req.session.role = sessao.cargo;
  req.session.permissoes = sessao.permissoes;
  req.session.permissions = sessao.permissoes;
  req.session.superadmin = sessao.superadmin;
  req.session.isSuperAdmin = sessao.superadmin;
  req.session.admin = sessao.superadmin;
