...
function usuariosDoArquivoLocal() {
  try {
    ...
    if (Array.isArray(bruto)) {
      lista = bruto;
    } else if (Array.isArray(bruto.usuarios)) {
      lista = bruto.usuarios;
    } else if (Array.isArray(bruto.users)) {
      lista = bruto.users;
    } else if (typeof bruto === "object" && bruto !== null) {
      lista = Object.values(bruto);
    }

    return lista.map(normalizarUsuario).filter(Boolean);
  } catch (error) {
    console.log("⚠️ Chat: não consegui ler data/usuarios.json:", error.message);
    return [];
  }
}
...
async function usuariosDoSupabase() {
  ...
  try {
    ...
    if (error) {
      console.log("⚠️ Chat: não consegui buscar cejas_usuarios:", error.message);
      return [];
    }

    return (data || []).map(normalizarUsuario).filter(Boolean);
  } catch (error) {
    console.log("⚠️ Chat: erro ao buscar usuários no Supabase:", error.message);
    return [];
  }
}
...
function registrarChatCejasApi(app) {
  ...
  app.get("/api/chat/usuarios", exigirUsuario, async (req, res) => {
    try {
      ...
      if (emails.length && isSupabaseConfigured()) {
  ...
  console.log("✅ Chat Interno CEJAS carregado com usuários locais e Supabase.");
}
