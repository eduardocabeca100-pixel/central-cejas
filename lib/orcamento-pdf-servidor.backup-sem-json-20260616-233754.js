...
function resolverPastaMesExistente(ano, mesIndex) {
  ...
  const encontrada = existentes.find((nome) => {
    const limpo = limparAcentos(nome).toLowerCase();

    return (
      limpo === alvoLimpo ||
      limpo === `${mesNumero} ${alvoLimpo}` ||
      limpo.endsWith(` ${alvoLimpo}`) ||
      limpo.includes(alvoLimpo)
    );
  });

  if (encontrada) return encontrada;

  // Padrão correto do seu servidor: mês escrito sem número.
  const nova = mesNome;
...
function registrarOrcamentoPdfServidor(app) {
  ...
  console.log("✅ Rota de PDF do orçamento no servidor carregada com mês existente.");
}
