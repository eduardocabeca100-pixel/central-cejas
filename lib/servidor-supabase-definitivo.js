...
function limparEnvServidorCejas(value) {
...
async function garantirBucket() {
  ...
  const { data, error } = await getSupabaseAdminServidor().storage.listBuckets();
  ...
  if (!existe) {
    const created = await getSupabaseAdminServidor().storage.createBucket(BUCKET, {
      public: false,
      fileSizeLimit: null
    });
...
async function uploadBuffer(relativePath, buffer) {
  ...
  const { error } = await getSupabaseAdminServidor().storage.from(BUCKET).upload(clean, buffer, {
    upsert: true,
...
async function downloadBuffer(relativePath) {
  ...
  const clean = limparPath(relativePath);
  const { data, error } = await getSupabaseAdminServidor().storage.from(BUCKET).download(clean);
...
async function listarStorage(prefix = "") {
  ...
  while (true) {
    const { data, error } = await getSupabaseAdminServidor().storage.from(BUCKET).list(folder, {
      limit,
      offset,
...
async function deletarItem(relativePath) {
  ...
  for (let i = 0; i < targets.length; i += 100) {
    ...
    const { error } = await getSupabaseAdminServidor().storage.from(BUCKET).remove(chunk);
...
async function moverItem(origem, destinoPasta) {
  ...
  if (arquivos.length) {
    ...
    for (const arquivo of arquivos) {
      ...
      const { error } = await getSupabaseAdminServidor().storage.from(BUCKET).move(arquivo, destinoArquivo);
  ...
  const { error } = await getSupabaseAdminServidor().storage.from(BUCKET).move(origemClean, destino);
