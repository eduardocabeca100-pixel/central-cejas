...
async function uploadBuffer(relativePath, buffer) {
  ...
  const { error } = await getSupabaseAdmin().storage.from(BUCKET).upload(clean, buffer, {
    upsert: true,
...
async function downloadBuffer(relativePath) {
  ...
  const clean = limparPath(relativePath);
  const { data, error } = await getSupabaseAdmin().storage.from(BUCKET).download(clean);
...
async function listarStorage(prefix = "") {
  ...
  while (true) {
    const { data, error } = await getSupabaseAdmin().storage.from(BUCKET).list(folder, {
      limit,
      offset,
...
async function moverItem(origem, destinoPasta) {
  ...
  if (arquivos.length) {
    ...
    for (const arquivo of arquivos) {
      ...
      const { error } = await getSupabaseAdmin().storage.from(BUCKET).move(arquivo, destinoArquivo);
  ...
  const { error } = await getSupabaseAdmin().storage.from(BUCKET).move(origemClean, destino);
